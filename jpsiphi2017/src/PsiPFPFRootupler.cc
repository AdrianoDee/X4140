/*
   Package:    PsiPFPFRootupler
   Class:      PsiPFPFRootupler
...
class PsiPFPFRootupler : public edm::EDAnalyzer {
   public:
      explicit PsiPFPFRootupler(const edm::ParameterSet&);
      ~PsiPFPFRootupler() override;
...
UInt_t PsiPFPFRootupler::isTriggerMatched(pat::CompositeCandidate *diMuon_cand) {
...
//
PsiPFPFRootupler::PsiPFPFRootupler(const edm::ParameterSet& iConfig):
...
PsiPFPFRootupler::~PsiPFPFRootupler() {}

//
// member functions
//

bool PsiPFPFRootupler::isAncestor(const reco::Candidate* ancestor, const reco::Candidate * particle) {
...
void PsiPFPFRootupler::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
//  using namespace edm;
  using namespace std;
...
// get rf information. Notice we are just keeping combinations with succesfull vertex fit
  if (jpsitrktrk_rf_cand_handle.isValid() && jpsitrktrk_cand_handle.isValid()) {
    ...
    for (unsigned int i=0; i< jpsitrktrk_rf_cand_handle->size(); i++){
      ...
      reco::Candidate::LorentzVector kP = phi_cand_rf->daughter("trakP")->p4();
      reco::Candidate::LorentzVector kM = phi_cand_rf->daughter("trakN")->p4();
      ...
      kP = phi_cand->daughter("trakP")->p4();
      kM = phi_cand->daughter("trakN")->p4();
      ...
      jpsi_triggerMatch = PsiPFPFRootupler::isTriggerMatched(onia_cand);
...
void PsiPFPFRootupler::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void PsiPFPFRootupler::endJob() {}

// ------------ method called when starting to processes a run  ------------
void PsiPFPFRootupler::beginRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a run  ------------
void PsiPFPFRootupler::endRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when starting to processes a luminosity block  ------------
void PsiPFPFRootupler::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a luminosity block  ------------
void PsiPFPFRootupler::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void PsiPFPFRootupler::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(PsiPFPFRootupler);
