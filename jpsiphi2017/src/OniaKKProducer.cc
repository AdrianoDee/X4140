#include "../interface/OniaKKProducer.h"
...
void OniaKKProducer::produce(edm::Event& event, const edm::EventSetup& esetup){
  ...
  event.getByToken(KaonCollection_,trak);
...
bool OniaKKProducer::IsTheSame(const pat::GenericParticle& tk, const pat::Muon& mu){
...
const pat::CompositeCandidate OniaKKProducer::makeOniaTTCandidate(
...
const pat::CompositeCandidate OniaKKProducer::makeTTCandidate(
...
reco::Candidate::LorentzVector OniaKKProducer::convertVector(const math::XYZTLorentzVectorF& v){

  return reco::Candidate::LorentzVector(v.x(),v.y(), v.z(), v.t());
}
//define this as a plug-in
DEFINE_FWK_MODULE(OniaKKProducer);
